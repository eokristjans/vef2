...
Hægt að nota `item.js`.

Þarf að sækja `jsdom` pakka til að nota HTML, DOM og `outerHTML()` á bakenda. Sjá að neðan.

`nodemon` (borið fram Node-Mon) server veldur því að server restartast í hvert skipti sem maður gerir breytingar.

Skoða betur eftir fyrirlestur 2 [kynningu verkefnis 1](https://www.youtube.com/watch?v=MQBTq1yuxBA&list=PLRj-ccg8iozy9xtBk-02VNOnOoIFR84Oe&index=5).

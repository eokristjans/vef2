...
Afritum texta úr _.env\_example_ í _.env_ og breytum `DATABASE_URL=postgres://:@localhost/v2-syni`
í `DATABASE_URL=postgres://:@localhost/v2-syni` fyrir sýnidæmi.
...
Hægt að tengjast í skel með `psql -U notandi -d v2_syni`

Setjum töflunafn sem _students_ í _schema.sql_ og i _setup.js_ (bara sýnidæmi). 
...
Bætum `app.use(register);` í _app.js_. Búum til skránna _register.js_ og importum hana í _app.js_.

Template _index.ejs_ á ekki að sjá um neina lógík. Öll lógík fer fram í middleware, hér í _register.js_.
...
TODO: Skoða vandamál með íslenska stafi í forriti, ekki í gagnagrunni.

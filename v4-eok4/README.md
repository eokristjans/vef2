...
Setja upp PSQL gagnagrunn (notaði bara v2, bætum nýrri töflu þangað inn).

Setja upp töflu í gagnagrunn og prufugögn í `/setup`.

Til að birta íslenska stafi í `psql` þurfti að keyra eftirfarandi í CMD:
...
Lærdómur af þessu:
- Hægt að laga margar eslint villur með `npm run eslint --fix` á skipanalinu.
- `const { sanitize } = require('express-validator');` er middleware og því bara hægt að nota þannig (ekki hægt að kalla beint á `sanitize(title).trim().blacklist(' ').escape()). Virðist líka vera útrunnið.
- `todos.js` sér um að hreinsa inntak (getur ekki treyst á router / `api.js` til að gera það fyrir sig).
- Þar með þarf að finna pakka sem er ekki middleware til að hreinsa, eða forrita það sjálfur. Pakkinn `const xss = require('xss');` sér reyndar um eitthvað af því (mögulega allt).
